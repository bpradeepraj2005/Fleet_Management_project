...
def load_vehicles():
    ...
    history_df = df.sort_values('timestamp')[['vehicle_id', 'timestamp', 'state_of_health_soh']]
    history_dict = {}
    for vid, group in history_df.groupby('vehicle_id'):
        history_dict[vid] = group[['timestamp', 'state_of_health_soh']].to_dict(orient='records')
        
    vehicles_list = vehicles_df.to_dict(orient='records')
    for v in vehicles_list:
        v['battery_history'] = history_dict.get(v['vehicle_id'], [])
...
def get_admin_dashboard_stats():
    ...
    trend_df = last_3m_df.groupby('date')[['daily_income', 'charging_cost']].sum().reset_index()
    trend_3m = trend_df.sort_values('date').to_dict(orient='records')
...
def get_admin_drivers_stats():
    ...
    history_df = df.sort_values('timestamp')[['driver_id', 'timestamp', 'performance_score', 'daily_income']]
    history_dict = {}
    for did, group in history_df.groupby('driver_id'):
        history_dict[did] = group[['timestamp', 'performance_score', 'daily_income']].to_dict(orient='records')
